...
class L2monitor(app_manager.RyuApp):
    ...
    # 初期値の入力
    s1_1_in_now = 0   #switch1 ポート1から入力されたパケット数
    s1_2_in_now = 0
    s2_3_in_now = 0
    s2_4_in_now = 0
    s1_1_out_now = 0  #switch1 ポート2から出力パケット数
    s1_2_out_now = 0   #switch2 ポート3から入力パケット数
    s2_3_out_now = 0  #switch1 ポート2から出力したパケット数
    s2_4_out_now = 0   #switch2 ポート3から出力したパケット数
    s1_out_ago = 0
    s2_in_ago = 0
...
#Flow Stats Replyを受け取ったときに実行される関数
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        if self.count == 1:
            if self.s1_2_out_now != self.s1_out_ago and self.s2_3_in_now != self.s2_in_ago:
                print("*************************************")            
                print("ポート1から入力: {}, ポート2から入力: {}, ポート3から入力: {}, ポート4から入力: {}".format(self.s1_1_in_now, self.s1_2_in_now, self.s2_3_in_now, self.s2_4_in_now))
                print("ポート1から出力: {}, ポート2から出力: {}, ポート3から出力: {}, ポート4から出力: {}".format(self.s1_1_out_now, self.s1_2_out_now, self.s2_3_out_now, self.s2_4_out_now))
                self.s1_out_ago = self.s1_2_out_now
                self.s2_in_ago = self.s2_3_in_now
                print("*************************************")
                print("")
            elif self.s1_2_out_now == self.s1_out_ago and self.s2_3_in_now == self.s2_in_ago:
                print("*************************************")
                print("パケットは流れていません")
                print("*************************************")
                print("")
